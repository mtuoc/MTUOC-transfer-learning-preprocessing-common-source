#    MTUOC-transfer-learning-preprocessing-common-source
...
stream = open('config-transfer-learning-preprocessing-common-source.yaml', 'r',encoding="utf-8")
...
split_corpusA(corpusA,valsizeA,evalsizeA,L1code3,L3code3)

trainCorpus="trainA-"+L1code3+"-"+L3code3+".txt"
valCorpus="valA-"+L1code3+"-"+L3code3+".txt"
evalCorpus="evalA-"+L1code3+"-"+L3code3+".txt"
trainPreCorpus="trainA-pre-"+L1code3+"-"+L3code3+".txt"
valPreCorpus="valA-pre-"+L1code3+"-"+L3code3+".txt"
evalSL="evalA."+L1code2
evalTL="evalA."+L3code2
...
#concateniating trainASL.temp and trainBSL.temp

outconcat=codecs.open("tempconcat.temp","w",encoding="utf-8")

entradaconcat=codecs.open("trainASL.temp","r",encoding="utf-8")
for linia in entradaconcat:
    linia=linia.rstrip()
    outconcat.write(linia+"\n")
entradaconcat.close()

entradaconcat=codecs.open("trainBSL.temp","r",encoding="utf-8")
for linia in entradaconcat:
    linia=linia.rstrip()
    outconcat.write(linia+"\n")
entradaconcat.close()

if TRAIN_L1_TRUECASER:
    if VERBOSE:
        cadena="Training L1 Truecaser: "+str(datetime.now())
        print(cadena)
        logfile.write(cadena+"\n")
    SLTrainer=TC_Trainer(MTUOC, L1_TC_MODEL, "tempconcat.temp", L1_DICT, L1_TOKENIZER)
    SLTrainer.train_truecaser()
os.remove("tempconcat.temp")

if TRAIN_L3_TRUECASER:
    if VERBOSE:
        cadena="Training L3 Truecaser: "+str(datetime.now())
        print(cadena)
        logfile.write(cadena+"\n")
    SLTrainer=TC_Trainer(MTUOC, L3_TC_MODEL, "trainATL.temp", L3_DICT, L3_TOKENIZER)
    SLTrainer.train_truecaser()  
    
if TRAIN_L2_TRUECASER:
    if VERBOSE:
        cadena="Training L2 Truecaser: "+str(datetime.now())
        print(cadena)
        logfile.write(cadena+"\n")
    SLTrainer=TC_Trainer(MTUOC, L2_TC_MODEL, "trainBTL.temp", L2_DICT, L2_TOKENIZER)
    SLTrainer.train_truecaser()  
...
if TRAIN_SENTENCEPIECE:
    ...
    trainCorpus="trainA-pre-"+L1code3+"-"+L3code3+".txt"
...
if VERBOSE:
    cadena="Encoding train A corpus "+L1code2+" with sentencepiece: "+str(datetime.now())
    print(cadena)
    logfile.write(cadena+"\n")

infile="trainA."+L1code2
outfile="trainA.sp."+L1code2
...
if VERBOSE:
    cadena="Encoding val A corpus "+L1code2+" with sentencepiece: "+str(datetime.now())
    print(cadena)
    logfile.write(cadena+"\n")

infile="valA."+L1code2
outfile="valA.sp."+L1code2
vocabulary_file="vocab_file."+L1code2
sentencepiece_encode(infile,outfile, SP_MODEL,vocabulary_file,VOCABULARY_THRESHOLD,bosSP,eosSP)
...
if GUIDED_ALIGNMENT:
    if VERBOSE:
        ...
        logfile.write(cadena+"\n")
    if DELETE_EXISTING:
        FILE="trainA.sp."+L1code2+"."+L3code2+".align" 
...
if GUIDED_ALIGNMENT_VALID:
    if VERBOSE:
        ...
        logfile.write(cadena+"\n")
    if DELETE_EXISTING:
        FILE="valA.sp."+L1code2+"."+L3code2+".align" 
        if os.path.exists(FILE):
            os.remove(FILE)
